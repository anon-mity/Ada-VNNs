...
from data_utils.ShapeNetSegmV0_Pose import PartNormalDataset
...
def parse_args():
    ...
    # model
    parser.add_argument('--model', default= 'vn_ori_globa6d',
                        choices=['dgcnn',
                                 'vn_dgcnn',
                                 'vn_ori_dgcnn','vn_ori_globa',
                                 'vn_ori_globa6d',
                                 'vn_ori_globa6d_lRlR',
                                 'vn_ori_globa6d_LRLR',
                                 'vn_ori_globa6d_nzt_lRlR',
                                 'vn_ori_globa6d_nzt_lRlRlR',
                                 ...
                                 'vn_transformer_am',
                                 'vn_transformer_nzt_w_lR',
                                 'vn_transformer_nzt_w_lRlR',
                                 'vn_transformer_amx3',
    ...
    parser.add_argument('--epoch', default=120, type=int, help='Number of epoch in training [default: 250]')
    ...
    # Dataset
    parser.add_argument('--data_choice', default='shapenetcore', choices=['shapenet', 'modelnet', 'shapenetseg', 'shapenetcore','scanobject', 'threedmatch','modelnet_40cate'])
    ...
    parser.add_argument('--shapenetseg_path', default='/home/hanbing/datasets/shapenetcore_partanno_segmentation_benchmark_v0_normal')
...
def train(trainDataLoader, testDataLoader, pose_model, logger, start_epoch, log_dir, checkpoints_dir, args):
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        with torch.no_grad():
            ...
            #test_writer.add_scalar('RR10', RR10, test_step)
...
def main(args, timestr):
    ...
    if args.data_choice == 'shapenet':
        ...
        categorys = ['can','bookshelf','jar','household','bath',"bus",'walkie-talkie','clock','faucet','guitar','laptop','mug','pot']
        #categorys = [ 'walkie-talkie', 'clock', 'faucet',
        #             'guitar', 'knife', 'laptop', 'mug', 'pot']

    elif args.data_choice == 'modelnet':
        categorys = ['all']
    elif args.data_choice == 'shapenetseg':
       # categorys = ['Airplane','Bag','Cap','Car','Chair','Earphone','Guitar','Knife','Lamp','Laptop','Motorbike','Mug','Pistol','Rocket','Skateboard','Table']
        categorys = ['Bag', 'Cap',  'Earphone', 'Guitar', 'Knife', 'Laptop',
        ...
        categorys = ['airplane', 'dresser', 'range_hood', 'bathtub', 'sink', 'vase', 'bed', 'flower_pot', 'sofa',
                     ...
                     'mantel', 'night_stand', 'cup', 'person', 'wardrobe', 'curtain', 'piano', 'xbox', 'desk', 'plant',
                     'door','radio']
    ...
    for i in range(len(categorys)):
        ...
        if args.data_choice == 'shapenet':
            ...
            TRAIN_DATASET = ShapeNetCoreH5Loader(data_path=args.shapenetcore_path, mode='train', category=args.category,
                                             num_pts=args.num_point)
            ...
            TRAIN_DATASET = PartNormalDataset(root=args.shapenetseg_path, split='trainval', class_choice=args.category,
                                               npoints=args.num_point)
            TEST_DATASET = PartNormalDataset(root=args.shapenetseg_path, split='test', class_choice=args.category,
                                              npoints=args.num_point)
            trainDataLoader = torch.utils.data.DataLoader(TRAIN_DATASET, batch_size=args.batch_size, shuffle=True,
                                                          num_workers=4, drop_last=False)
            testDataLoader = torch.utils.data.DataLoader(TEST_DATASET, batch_size=args.batch_size, shuffle=True,
            ...
            TRAIN_DATASET = ModelNetDataLoader_40cate(root=args.modelnet_path, npoint=args.num_point, split='train',
                                               normal_channel=args.normal, category_choice=args.category)
