...
class ModelNetDataLoader_40cate_latentvis(Dataset):
    def __init__(self, root,  npoint=1024, split='train', uniform=False, normal_channel=False, category_choice=None, cache_size=15000):
        ...
        # 筛选出在category_choice中的数据
        valid_indices = [i for i, name in enumerate(shape_names) if name in category_choice]
        shape_names = [shape_names[i] for i in valid_indices]
        shape_ids_split = [shape_ids[split][i] for i in valid_indices]

        # list of (shape_name, shape_txt_file_path) tuple
        # 构建筛选后的datapath
        self.datapath = [(shape_names[i], os.path.join(self.root, shape_names[i], shape_ids_split[i]) + '.txt')
    ...
    def _get_item(self, index):
        ...
        pc_norm = point_set
        pc_so3_1, R_so3_1 = rotate_point_cloud(point_set[:, 0:3])  # so3
        pc_so3_2, R_so3_2 = rotate_point_cloud(point_set[:, 0:3])  # so3
        pc_so3_3, R_so3_3 = rotate_point_cloud(point_set[:, 0:3])  # so3

        return {
            'pc_norm': torch.from_numpy(pc_norm.astype(np.float32)),
            'pc_so3_1': torch.from_numpy(pc_so3_1.astype(np.float32)),
            'target_so3_1': torch.from_numpy(R_so3_1.astype(np.float32)),
            'pc_so3_2': torch.from_numpy(pc_so3_2.astype(np.float32)),
            'target_so3_2': torch.from_numpy(R_so3_2.astype(np.float32)),
            'pc_so3_3': torch.from_numpy(pc_so3_3.astype(np.float32)),
            'target_so3_3': torch.from_numpy(R_so3_3.astype(np.float32)),
        }
