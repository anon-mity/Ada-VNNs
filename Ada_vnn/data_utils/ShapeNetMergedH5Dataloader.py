...
class ShapeNetMergedH5Loader(Dataset):
    def __init__(self, data_path, number='10percent', category='', num_pts=1024):
        """
        Args:
        ...
        """
        if number is None:
            self.file_path = os.path.join(data_path, f'{category}.h5')
        else:
            self.file_path = os.path.join(data_path, f'{category}_{number}.h5')
